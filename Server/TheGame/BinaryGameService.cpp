...
BinaryGameService::ActionResult BinaryGameService::ProcessPlayerAction(Game& game, int userId, const std::string& binaryData)
{
    ...
    if (!msg.ParseFromString(binaryData)) return { false, "Failed to parse action message", false };
    if (msg.type() != myproject::GameMessage::ACTION || !msg.has_action()) return { false, "Invalid message type for action", false };
    const myproject::PlayerAction& action = msg.action();
    if (action.action_type() == myproject::PlayerAction::PLAY_CARD) {
        if (game.PlaySingleCard(userId, action.card_value(), action.stack_index())) return { true, "", false };
        else return { false, "Invalid move", false };
    }
    else if (action.action_type() == myproject::PlayerAction_ActionType_END_TURN) {
        if (game.EndCurrentTurn(userId)) return { true, "", true };
        else return { false, "Cannot end turn now", false };
    }
...
std::vector<std::pair<int, std::string>> BinaryGameService::PrepareBroadcastMessages(Game& game)
{
    ...
    for (const auto& targetPlayer : players) {
        ...
        msgWrapper.set_type(currentStatus != GameState::InProgress ? myproject::GameMessage_Type_GAME_OVER : myproject::GameMessage_Type_STATE_UPDATE);
        ...
        for (const auto& p : players) {
            ...
            if (p.GetId() == targetPlayer.GetId()) {
                for (uint8_t cardVal : p.GetDeck()) pInfo->add_your_cards(cardVal);
            }
        }
